...
citiesLayer=new carto.layer.Layer(citiesSource,citiesStyle,{featureClickColumns:['name','papers']});updatePerYearPlot();updatePerJournalPlot();updatePerTopicPlot();hidePlots()}
if(options.length===0){citiesSource.setQuery(query({cities:500,year:['9999'],journal:selectedJournalsArray,edamcat:selectedTopicsArray}))
citiesLayer=new carto.layer.Layer(citiesSource,citiesStyle,{featureClickColumns:['name','papers']});selectedYearsArray=['9999'];return'0 years selected'}
else{return options.length+' years selected'}}},'selectAll');$('#journalsSelector').multiselect({includeSelectAllOption:!0,allSelectedText:'All journals selected',buttonWidth:'200px',onChange:function(option,checked){},buttonText:function(options){if(options.length>0){var selected=[];options.each(function(){selected.push([$(this).text(),$(this).data('order')])});selected.sort(function(a,b){return a[1]-b[1]});var text='';for(var i=0;i<selected.length;i++){text+="'"+selected[i][0]+"'"+', '}
...
citiesLayer=new carto.layer.Layer(citiesSource,citiesStyle,{featureClickColumns:['name','papers']});updatePerYearPlot();updatePerJournalPlot();updatePerTopicPlot();hidePlots()}
if(options.length===0){citiesSource.setQuery(query({cities:500,year:['9999'],journal:selectedJournalsArray,edamcat:selectedTopicsArray}))
citiesLayer=new carto.layer.Layer(citiesSource,citiesStyle,{featureClickColumns:['name','papers']});selectedJournalsArray=['9999'];return'0 journals selected'}
else{return options.length+' journals selected'}}},'selectAll');$('option',$('#journalsSelector')).each(function(index,element){$(this).removeAttr('selected').prop('selected',!0)});$("#journalsSelector").multiselect('refresh');$('option',$('#yearSelector')).each(function(index,element){if(index<6){$(this).removeAttr('selected').prop('selected',!1)}
if(index>4){$(this).removeAttr('selected').prop('selected',!0)}
if(index>9){$(this).removeAttr('selected').prop('selected',!1)}});$("#yearSelector").multiselect('refresh')});const popup=L.popup({closeButton:!0});function openPopup(featureEvent){popup.setLatLng(featureEvent.latLng);if(!popup.isOpen()){let content='';content+=`<div class="popup-container" id="institutionWindow">`;if(featureEvent.data.name){var query="SELECT SUM(CAST(num_papers AS INT)) FROM results_full_280218_v1 WHERE nameaffiliation = '"+featureEvent.data.name+"' AND source IN ("+selectedJournalsArray+") AND edamcategory IN ("+selectedTopicsArray+") AND year IN ("+selectedYearsArray+")";SQL_CLIENT.request({params:{q:query},}).then(function(response){if(response&&response.data){var numberPapers=response.data.rows[0].sum;content+=`<h4>INSTITUTION: ${featureEvent.data.name}</h4>`;content+=`<br>`;content+=`<p><b>PAPERS ON THIS PERIOD:</b> ${numberPapers}</p>`;content+=`<canvas id="bar-chart" width="400" height="250"></canvas><br>`;content+=`<canvas id="insPerTopicPieChart" width="500" height="250"></canvas>`;content+=`</div>`;popup.setContent(content);popup.openOn(map);var numberWithCommas=function(x){return x.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")};var dataPack1=[40];var dataPack2=[10];var dataPacks=[];var dates=[featureEvent.data.year];var query="SELECT source,year,SUM(CAST(num_papers AS INT)) FROM results_full_280218_v1 WHERE nameaffiliation = '"+featureEvent.data.name+"' AND source IN ("+selectedJournalsArray+") AND year IN ("+selectedYearsArray+") AND edamcategory IN ("+selectedTopicsArray+") GROUP BY year,source ORDER by year ASC";SQL_CLIENT.request({params:{q:query},}).then(function(response){if(response&&response.data){dataArray=[];labelsArray=[];var journalsArray=[]
...
for(var j=0;j<response.data.rows.length;j++){if(response.data.rows[j].source==journalsArray[i]){for(var l=0;l<tempDataArray.length;l++){if(parseInt(response.data.rows[j].year,10)==tempDataArray[l].year)
